...
import 'package:awesome_period_tracker/config/gemini_client.dart';
  ...
  Future<Insight> getInsightForForecast({
    ...
    try {
      if (_sharedPreferences.containsKey(prefsKey) && useCache) {
        final cachedInsight = Insight.fromJson(
          jsonDecode(_sharedPreferences.getString(prefsKey)!),
        );

        final isCacheValid =
            cachedInsight.date.isSameDay(forecast.date.toUtc());
    ...
    const isPast = false; // TODO Make this dynamic
    ...
    await _sharedPreferences.setString(prefsKey, jsonEncode(insight.toJson()));
